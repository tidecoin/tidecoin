    ...
    return RPCHelpMan{
        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue {
            ...
            if (interfaces::Ipc* ipc = local_init.ipc()) {
                // Spawn a new tidecoin-node process and call makeEcho to get a
                ...
                // and spawn bitcoin-echo below instead of tidecoin-node. But
                // using tidecoin-node avoids the need to build and install a
                // new executable just for this one test.
                auto init = ipc->spawnProcess("tidecoin-node");
                echo = init->makeEcho();
                ipc->addCleanup(*echo, [init = init.release()] { delete init; });
            } else {
                // IPC support is not available because this is a tidecoind
                // process not a tidecoind-node process, so just create a local
