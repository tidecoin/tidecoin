...
Options:
  -m, --multiprocess     Run multiprocess binaries tidecoin-node, tidecoin-gui.
  -M, --monolithic       Run monolithic binaries tidecoind, tidecoin-qt. (Default behavior)
  -v, --version          Show version information
  -h, --help             Show full help message

Commands:
  gui [ARGS]     Start GUI, equivalent to running 'tidecoin-qt [ARGS]' or 'tidecoin-gui [ARGS]'.
  node [ARGS]    Start node, equivalent to running 'tidecoind [ARGS]' or 'tidecoin-node [ARGS]'.
  rpc [ARGS]     Call RPC method, equivalent to running 'tidecoin-cli -named [ARGS]'.
  wallet [ARGS]  Call wallet command, equivalent to running 'tidecoin-wallet [ARGS]'.
  tx [ARGS]      Manipulate hex-encoded transactions, equivalent to running 'tidecoin-tx [ARGS]'.
  help           Show full help message.
)";

static constexpr auto HELP_FULL = R"(
Additional less commonly used commands:
  bench [ARGS]      Run bench command, equivalent to running 'bench_tidecoin [ARGS]'.
  chainstate [ARGS] Run tidecoin kernel chainstate util, equivalent to running 'tidecoin-chainstate [ARGS]'.
  test [ARGS]       Run unit tests, equivalent to running 'test_tidecoin [ARGS]'.
  test-gui [ARGS]   Run GUI unit tests, equivalent to running 'test_tidecoin-qt [ARGS]'.
)";
...
int main(int argc, char* argv[])
{
    try {
        ...
        if (cmd.show_help || cmd.command.empty()) {
            ...
            args.emplace_back(cmd.use_multiprocess ? "tidecoin-gui" : "tidecoin-qt");
        } else if (cmd.command == "node") {
            args.emplace_back(cmd.use_multiprocess ? "tidecoin-node" : "tidecoind");
        } else if (cmd.command == "rpc") {
            args.emplace_back("tidecoin-cli");
            // Since "tidecoin rpc" is a new interface that doesn't need to be
            ...
            args.emplace_back("tidecoin-wallet");
        } else if (cmd.command == "tx") {
            args.emplace_back("tidecoin-tx");
        } else if (cmd.command == "bench") {
            args.emplace_back("bench_tidecoin");
        } else if (cmd.command == "chainstate") {
            args.emplace_back("tidecoin-chainstate");
        } else if (cmd.command == "test") {
            args.emplace_back("test_tidecoin");
        } else if (cmd.command == "test-gui") {
            args.emplace_back("test_tidecoin-qt");
        } else if (cmd.command == "util") {
            args.emplace_back("tidecoin-util");
        } else {
...
//! Execute the specified tidecoind, tidecoin-qt or other command line in `args`
...
//!             be a relative path to a tidecoind, tidecoin-qt or other executable
...
//!
//!     strace -e trace=execve -s 10000 build/bin/tidecoin ...
