# Multiprocess Tidecoin Design Document

Guide to the design and architecture of Tidecoin's multiprocess feature.
...
Tidecoin has historically employed a monolithic architecture inherited from its
upstream codebase. This design integrates functionality like P2P network operations,
wallet management, and GUI support into single executables. While effective, it
has limitations in flexibility, security, and scalability. The multiprocess
work introduces a modular architecture to improve isolation, simplify
maintenance, and support more flexible deployment models.

## Current Architecture

The current system features two primary monolithic executables: `tidecoind` and
`tidecoin-qt`. `tidecoind` combines a Tidecoin P2P node with an integrated
JSON-RPC server, wallet, and indexes. `tidecoin-qt` extends this by
incorporating a Qt-based GUI.

With `-DENABLE_IPC=ON`, supplemental executables (`tidecoin-node`,
`tidecoin-gui`) are also built and can be launched through the `tidecoin -m`
wrapper. Today they are mostly behavior-compatible with monolithic binaries and
primarily expose IPC plumbing.

## Target Architecture

The target architecture divides code into three specialized executables:

- `tidecoin-node`: Manages the P2P node, indexes, and JSON-RPC server.
- `tidecoin-wallet`: Handles all wallet functionality.
- `tidecoin-gui`: Provides a standalone Qt-based GUI.
...
This subdivision could be extended in the future. For example, indexes could be removed from the `tidecoin-node` executable and run in separate executables. And JSON-RPC servers could be added to wallet and index executables, so they can listen and respond to RPC requests on their own ports, without needing to forward RPC requests through `tidecoin-node`.

<table><tr><td>

```mermaid
flowchart LR
    node[tidecoin-node] -- listens on --> socket["&lt;datadir&gt;/node.sock"]
    wallet[tidecoin-wallet] -- connects to --> socket
    gui[tidecoin-gui] -- connects to --> socket
```
...
The choice to use an RPC framework at all instead of a custom protocol was
necessitated by the size of Tidecoin internal interfaces (inherited from
upstream architecture), which consist of around 150 methods that pass complex
data structures and are called in complicated ways (in parallel, and from
callbacks that can be nested and stored). Writing a custom protocol to wrap
these interfaces would be a lot more work, akin to writing a new RPC framework.

### Hiding IPC

The IPC mechanism is deliberately isolated from the rest of the codebase so less code has to be concerned with IPC.

Building Tidecoin with IPC support is optional, and node, wallet, and GUI code
can be compiled to either run in the same process or separate processes. The
build system also ensures Cap’n Proto library headers can only be used within
the [`src/ipc/capnp/`](../../src/ipc/capnp/) directory, not in other parts of
the codebase.
...
The integration of [Cap’n Proto](https://capnproto.org/) and
[libmultiprocess](https://github.com/bitcoin-core/libmultiprocess) into
Tidecoin increases potential attack surface. Cap’n Proto, being a substantial
dependency, introduces potential sources of vulnerability, particularly through
the creation of new UNIX sockets. The inclusion of libmultiprocess also
contributes to this risk.

In Tidecoin, libmultiprocess is vendored under
[`src/ipc/libmultiprocess/`](../../src/ipc/libmultiprocess/) by default, with
an option to use an external installation. IPC support can also be disabled at
build time, allowing operators to choose between maximal isolation features and
...
Let’s walk through an example where the `tidecoin-wallet` process requests the hash of a block at a specific height from the `tidecoin-node` process. This example demonstrates the practical application of the IPC mechanism, specifically the interplay between C++ method calls and Cap’n Proto-generated RPC calls.

<table><tr><td>

```mermaid
sequenceDiagram
    box "tidecoin-wallet process"
...
1. **Initiation in tidecoin-wallet**
...
2. **Translation to Cap’n Proto RPC**
   ...
   - The client subclass is automatically generated by the `mpgen` tool from Cap'n Proto schema files in [`src/ipc/capnp/`](../../src/ipc/capnp/).

3. **Request Preparation and Dispatch**
   - The `getBlockHash` method of the generated `Chain` client subclass in `tidecoin-wallet` populates a Cap’n Proto request with the `height` parameter, sends it to `tidecoin-node` process, and waits for a response.

4. **Handling in tidecoin-node**
   - Upon receiving the request, the Cap'n Proto dispatching code in the `tidecoin-node` process calls the `getBlockHash` method of the `Chain` [server class](#c-server-classes-in-generated-code).
   - The server class is automatically generated by the `mpgen` tool from Cap'n Proto schema files in [`src/ipc/capnp/`](../../src/ipc/capnp/).
   - The `getBlockHash` method of the generated `Chain` server subclass in `tidecoin-node` receives a Cap’n Proto request object with the `height` parameter, and calls the `getBlockHash` method on its local `Chain` object with the provided `height`.
   - When the call returns, it encapsulates the return value in a Cap’n Proto response, which it sends back to the `tidecoin-wallet` process.

5. **Response and Return**
   - The `getBlockHash` method of the generated `Chain` client subclass in `tidecoin-wallet` which sent the request now receives the response.
...
- Separating indexes from `tidecoin-node`, and running indexing code in separate processes (see [indexes: Stop using node internal types #24230](https://github.com/bitcoin/bitcoin/pull/24230)).
...
- Using Cap'n Proto's support for [other languages](https://capnproto.org/otherlang.html), such as [Rust](https://github.com/capnproto/capnproto-rust), to allow code written in other languages to call Tidecoin C++ code, and vice versa (see [How to rustify libmultiprocess? #56](https://github.com/bitcoin-core/libmultiprocess/issues/56)).

## Conclusion

This modularization represents an advancement in Tidecoin's architecture,
offering enhanced security, flexibility, and maintainability.

## Appendices

### Glossary of Terms

- **abstract class**: A class in C++ that consists of virtual functions. In Tidecoin, they define interfaces for inter-component communication.
...
- **client class (in generated code)**: A C++ class generated from a Cap’n Proto interface which inherits from a Tidecoin abstract class, and implements each virtual method to send IPC requests to another process. (see also [components section](#c-client-subclasses-in-generated-code))

- **IPC (inter-process communication)**: Mechanisms that enable processes to exchange requests and data.

- **ipc::Exception class**: A class within Tidecoin's protocol-agnostic IPC code that is thrown by client class methods when there is an IPC error.
...
- **RPC (remote procedure call)**: A protocol that enables a program to request a service from another program in a different address space or network. Tidecoin uses [JSON-RPC](https://en.wikipedia.org/wiki/JSON-RPC) for RPC.

- **server class (in generated code)**: A C++ class generated from a Cap’n Proto interface which handles requests sent by a _client class_ in another process. The request handled by calling a local Tidecoin interface method, and the return values (if any) are sent back in a response. (see also: [components section](#c-server-classes-in-generated-code))
...
This document is adapted from upstream multiprocess design work and reviewed for
Tidecoin naming and repository structure.
