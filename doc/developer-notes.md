...
Tidecoin uses [Doxygen](https://www.doxygen.nl/) to generate its official documentation.
...
configuration options and toggled while tidecoind is running with the `logging`
RPC.  For instance, launching tidecoind with `-debug` or `-debug=1` will turn on
...
you can run with the `-testnet` config option to test
with test coins on a test network.
...
Tidecoin is a multi-threaded application, and deadlocks or other
...
You can then use the `-debug=lock` configuration option at tidecoind startup or
`tidecoin-cli logging '["lock"]'` at runtime to turn on lock contention logging.
It can be toggled off again with `tidecoin-cli logging [] '["lock"]'`.
...
```shell
$ valgrind --suppressions=contrib/valgrind.supp build/bin/test_tidecoin
$ valgrind --suppressions=contrib/valgrind.supp --leak-check=full \
      --show-leak-kinds=all build/bin/test_tidecoin --log_level=test_suite
$ valgrind -v --leak-check=full build/bin/tidecoind -printtoconsole
...
# A coverage report will now be accessible at `./build/test_tidecoin.coverage/index.html`,
...
```shell
llvm-cov show \
    --object=build/bin/test_tidecoin \
    --object=build/bin/tidecoind \
    ...
    --project-title="Tidecoin Coverage Report"
```
...
llvm-cov show \
    ...
    --show-expansions \
    --output-dir=build/coverage_report \
    --project-title="Tidecoin Fuzz Coverage Report"
```
...
To profile a running tidecoind process for 60 seconds, you could use an
invocation of `perf record` like this:

```sh
$ perf record \
    -g --call-graph dwarf --per-thread -F 140 \
    -p `pgrep tidecoind` -- sleep 60
```
...
Tidecoin can be compiled with various "sanitizers" enabled, which add
...
pay attention to for reviewers of Tidecoin code.
...
- For `strprintf`, `LogInfo`, `LogDebug`, etc formatting characters don't need size specifiers (hh, h, l, ll, j, z, t, L) for arithmetic types.
  ...
  - *Rationale*: Tidecoin uses tinyformat, which is type safe. Leave them out to avoid confusion.
...
Normally, these are maintained by active developers of Tidecoin, in which case
...
our local subtree copy, so that they can be integrated quickly. Cosmetic changes
...
The tool instructions also include a list of the subtrees managed in this repository.
...
- src/leveldb
  - Upstream at https://github.com/google/leveldb ; maintained by Google. Open
    important PRs to the Tidecoin subtree to avoid delay.
...
## Vendored PQClean components

Post-quantum implementations in `src/pq/` are vendored from PQClean, but this
repository also carries explicit local integration patches (for deterministic
keygen entrypoints, include-path wiring, and Tidecoin adapter integration).

Governance artifacts:

- `src/pq/VERSIONS.toml`: canonical provenance and patch metadata.
- `src/pq/patches/`: local patch snapshots referenced by the manifest.
- `test/lint/lint-pq-vendor.py`: fast schema/metadata lint gate.
- `contrib/devtools/check_pq_vendor.py`: deep upstream-vs-local diff checker.

When modifying vendored PQ code:

1. Update `src/pq/VERSIONS.toml` in the same PR.
2. Update patch snapshots under `src/pq/patches/` for any changed component.
3. Run:
   - `test/lint/lint-pq-vendor.py`
   - `python3 contrib/devtools/check_pq_vendor.py`
4. Include a short PR note with affected component names, patch reason, and
   whether consensus behavior is impacted.
...
file descriptors, it will cause problems with Tidecoin's `select()` loop, because
...
compatibility between nodes. This happened in upstream Bitcoin Core 0.8 (when LevelDB was
...
would be to revert the upstream fix before applying the updates to Tidecoin's
...
- Try not to overload methods on argument type. E.g. don't make `getblock(true)` and `getblock("hash")`
  ...
  - *Rationale*: This is impossible to use with `tidecoin-cli`, and can be surprising to users.
...
- Add every non-string RPC argument `(method, idx, name)` to the table `vRPCConvertParams` in `rpc/client.cpp`.
  ...
  - *Rationale*: `tidecoin-cli` and the GUI debug console use this table to determine how to
...
- Wallet RPCs call BlockUntilSyncedToCurrentChain to maintain consistency with
  ...
  - *Rationale*: In previous versions of Tidecoin, the wallet was always
