...
Tidecoin has an internal benchmarking framework, with benchmarks
...
After compiling Tidecoin, the benchmarks can be run with:
    ...
    build/bin/bench_tidecoin

Or through the wrapper command:

    build/bin/tidecoin bench
...
To monitor node performance more in depth (like reindex or IBD), see:
