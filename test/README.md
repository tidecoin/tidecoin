This directory contains integration tests that test tidecoind and its
...
tidecoind and tidecoin-qt by interacting with them through the RPC and P2P
interfaces.
...
The P2P and RPC ports used by the tidecoind nodes-under-test are chosen to make
conflicts with other processes unlikely. However, if there is another tidecoind
process running on the system (perhaps from a previous test which hasn't successfully
killed all its tidecoind nodes), then there may be a port conflict which will
cause the test to fail. It is recommended that you run the tests on a system
where no other tidecoind processes are running.

On linux, the test framework will warn if there is another
tidecoind process running when the tests are started.

If there are zombie tidecoind processes after test failure, you can kill them
by running the following commands. **Note that these commands will kill all
tidecoind processes running on the system, so should not be used if any non-test
tidecoind processes are being run.**
...
sure tidecoind processes are stopped as above):
...
- when run by [our CI (Continuous Integration)](/ci/README.md), no logs are output to the console. However, if a test
  fails, the `test_framework.log` and tidecoind `debug.log`s will all be dumped
...
`test_framework.log` and tidecoind `debug.log`s can be combined into a single
...
call methods that interact with the tidecoind nodes-under-test.

If further introspection of the tidecoind instances themselves becomes
...
```bash
cat /tmp/user/1000/testo9vsdjo3/node1/regtest/tidecoind.pid
```

Then you can use the pid to start `gdb`:

```bash
gdb /home/example/tidecoind <pid>
```
