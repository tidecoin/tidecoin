    ...
    def run_test(self):
        ...
        for node in self.nodes[:2]:
            with node.assert_debug_log(['limited pruning to height 689']):
                ...
                assert pruneheight_new in (247, 248)
        ...
        for i in range(3):
            pruneheight_2 = self.nodes[i].pruneblockchain(index_height)
            assert_greater_than(pruneheight_2, 400)
            assert pruneheight_2 <= index_height
        ...
        for node in self.nodes[:2]:
            with node.assert_debug_log(['limited pruning to height 2489']):
                ...
                assert pruneheight_new >= 2005
                assert pruneheight_new <= 2025
