...
class InitTest(BitcoinTestFramework):
    ...
    def init_pid_test(self):
        TIDECOIN_PID_FILENAME_CUSTOM = "my_fancy_tidecoin_pid_file.foobar"

        self.log.info("Test specifying custom pid file via -pid command line option")
        custom_pidfile_relative = TIDECOIN_PID_FILENAME_CUSTOM
        ...
        assert not (datadir / TIDECOIN_PID_FILENAME_DEFAULT).exists()
        assert (datadir / custom_pidfile_relative).exists()
        self.stop_node(0)
        assert not (datadir / custom_pidfile_relative).exists()

        custom_pidfile_absolute = Path(self.options.tmpdir) / TIDECOIN_PID_FILENAME_CUSTOM
