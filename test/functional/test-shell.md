...
* Manage regtest tidecoind subprocesses.
* Access RPC interfaces of the underlying tidecoind instances.
...
* Python3
* `tidecoind` built in the same repository as the `TestShell`.
...
The following `TestShell` methods manage the lifetime of the underlying tidecoind
...
temporary folder. If you need more tidecoind nodes than set by default (1),
...
During the time between the `setup` and `shutdown` calls, all `tidecoind` node
...
Shutting down the `TestShell` will safely tear down all running tidecoind
...
The following utility consolidates logs from the tidecoind nodes and the
...
|---|---|---|
| `bind_to_localhost_only` | `True` | Binds tidecoind P2P services to `127.0.0.1` if set to `True`.|
| `cachedir` | `"/path/to/tidecoin/build/test/cache"` | Sets the tidecoind datadir directory. |
| `chain`  | `"regtest"` | Sets the chain-type for the underlying test tidecoind processes. |
| `configfile` | `"/path/to/bitcoin/build/test/config.ini"` | Sets the location of the test framework config file. |
| `coveragedir` | `None` | Records tidecoind RPC test coverage into this directory if set. |
| `loglevel` | `INFO` | Logs events at this level and higher. Can be set to `DEBUG`, `INFO`, `WARNING`, `ERROR` or `CRITICAL`. |
| `nocleanup` | `False` | Cleans up temporary test directory if set to `True` during `shutdown`. |
| `noshutdown` | `False` | Does not stop tidecoind instances after `shutdown` if set to `True`. |
| `num_nodes` | `1` | Sets the number of initialized tidecoind processes. |
| `perf` | False | Profiles running nodes with `perf` for the duration of the test if set to `True`. |
| `rpc_timeout` | `60` | Sets the RPC server timeout for the underlying tidecoind processes. |
| `setup_clean_chain` | `False` | A 200-block-long chain is initialized from cache by default. Instead, `setup_clean_chain` initializes an empty blockchain if set to `True`. |
| `randomseed` | Random Integer | `TestShell().options.randomseed` is a member of `TestShell` which can be accessed during a test to seed a random generator. User can override default with a constant value for reproducible test runs. |
| `supports_cli` | `False` | Whether the tidecoin-cli utility is compiled and available for the test. |
| `tmpdir` | `"/var/folders/.../"` | Sets directory for test logs. Will be deleted upon a successful test run unless `nocleanup` is set to `True` |
| `trace_rpc` | `False` | Logs all RPC calls if set to `True`. |
| `usecli` | `False` | Uses the tidecoin-cli interface for all tidecoind commands instead of directly calling the RPC server. Requires `supports_cli`. |
