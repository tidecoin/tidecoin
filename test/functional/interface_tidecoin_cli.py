...
WALLET_NOT_SPECIFIED = (
    ...
    "option before the command (run \"tidecoin-cli -h\" for help or \"tidecoin-cli listwallets\" to see "
...
class TestBitcoinCli(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        self.log.info("Compare responses from getblockchaininfo RPC and `tidecoin-cli getblockchaininfo`")
        ...
        # Prevent tidecoin-cli from using existing rpcport in conf
        ...
        self.log.info("Test -getinfo and tidecoin-cli return all proxies")
        ...
        if self.is_wallet_compiled():
            self.log.info("Test -getinfo and tidecoin-cli getwalletinfo return expected wallet info")
            ...
            assert_equal(Decimal(cli_get_info['Transaction fee rate (-paytxfee) (TDC/kvB)']), wallet_info['paytxfee'])
            assert_equal(Decimal(cli_get_info['Min tx relay fee rate (TDC/kvB)']), network_info['relayfee'])
            ...
            # Test tidecoin-cli -generate with -rpcwallet in multiwallet mode.
