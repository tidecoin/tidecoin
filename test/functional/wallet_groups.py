...
class WalletGroupTest(BitcoinTestFramework):
    def set_test_params(self):
        ...
        self.extra_args = [
            ...
            [],
            ["-avoidpartialspends"],
            ["-maxapsfee=0.00017359"],
            ["-maxapsfee=0.00017360"],
        ]
    ...
    def run_test(self):
        ...
        v.sort()
        assert_approx(v[0], vexp=0.2, vspan=0.0002)
        ...
        v.sort()
        assert_approx(v[0], vexp=0.2, vspan=0.0002)
        assert_approx(v[1], vexp=1.3, vspan=0.0002)
        ...
        # - C1 0.5      - F  ~1.3
        # - D ~0.3
        assert_approx(self.nodes[1].getbalance(), vexp=4.3, vspan=0.0002)
        assert_approx(self.nodes[2].getbalance(), vexp=4.3, vspan=0.0002)
        ...
        values.sort()
        assert_approx(values[0], vexp=0.1, vspan=0.0002)
        assert_approx(values[1], vexp=1.4, vspan=0.0002)
        ...
        self.nodes[0].sendtoaddress(addr_aps, 1.0)
        self.generate(self.nodes[0], 1)
        with self.nodes[3].assert_debug_log(['using grouped']):
        ...
        self.generate(self.nodes[0], 1)
        with self.nodes[4].assert_debug_log(['using grouped']):
