...
- blocks are mined back-to-back by the node itself so the first retarget is
  in the minimum-timespan clamp region.
- vectors are generated by the node itself (via `generateblock`), so PoW and
  coinbase/witness structure exactly match consensus.
