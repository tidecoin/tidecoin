...
def main():
    ...
    fuzz_bin=os.getenv("TIDECOINFUZZ", default=os.path.join(config["environment"]["BUILDDIR"], 'bin', 'fuzz'))
    ...
    if not args.generate:
        ...
        if test_list_missing_corpus:
            ...
            logging.info("Please consider adding Tidecoin-specific fuzz corpus inputs.")
...
def merge_inputs(*, fuzz_pool, corpus, test_list, src_dir, fuzz_bin, merge_dirs):
    ...
    for t in test_list:
        args = [
            ...
            # to cut the runtime to iterate over all fuzz inputs.
            '-shuffle=0',
            '-prefer_small=1',
            '-use_value_profile=0',
            # use_value_profile is enabled by oss-fuzz [0], but disabled for
            # now to avoid bloating the qa-assets git repository.
